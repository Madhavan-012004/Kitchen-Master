...
async function startPostgres() {
  updateSplash('Checking database engine…', 10);
  ...
  if (!fs.existsSync(path.join(DB_DIR, 'PG_VERSION'))) {
    ...
    updateSplash('Setting up local database (first run)…', 20);
    execSync(
  ...
  updateSplash('Starting local database engine…', 30);
  const startLog = path.join(LOG_DIR, 'postgres.log');
  execFile(pgCtlExe, ['start', '-D', DB_DIR, '-l', startLog]);
...
function startBackend() {
  return new Promise((resolve, reject) => {
    updateSplash('Starting backend services…', 45);
    ...
    javaProcess.stdout.on('data', d => {
      ...
      log('[Java] ' + line);
      if (line.includes('Started ProBloom')) updateSplash('Application ready!', 90);
    });
    ...
    // Poll /api/status until it responds 200 (max 90 s)
    let attempts = 0;
    const poll = () => {
      http.get(`${API_BASE}/status`, res => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          else reject('Backend failed to become healthy within 90 seconds.');
        }
      }).on('error', () => {
        if (++attempts < 90) setTimeout(poll, 1000);
        else reject('Backend did not respond within 90 seconds.');
      });
    };
    poll();
  });
...
function openMainWindow() {
  updateSplash('Loading ProBloom…', 100);
