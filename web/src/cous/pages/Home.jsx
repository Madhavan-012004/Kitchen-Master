...
export default function Home() {
    ...
    const handleFeedbackSubmit = async () => {
        ...
        try {
            await api.patch(`/orders/public/${placedOrderId}/feedback`, { rating, feedback: feedbackText });
            setFeedbackSubmitted(true);
            setTimeout(() => {
                setActiveOrder(prev => ({ ...prev, rating, feedback: feedbackText }));
            }, 500);
        } catch (err) {
                ...
                {activeOrder && (
                    ...
                    >
                        ...
                        <div className="tracker-progress">
                            {activeOrder.waitingWaiterAck ? (
                                [
                                    { icon: '🔔', label: 'Waiting for Waiter', statuses: ['PENDING'] },
                                ...
                                [
                                    { icon: '🍳', label: 'Preparing', statuses: ['PENDING','PREPARING','READY','SERVED','PAID'] },
                                    { icon: '🛎️', label: 'Ready', statuses: ['READY','SERVED','PAID'] },
                                    { icon: '🍽️', label: 'Served', statuses: ['SERVED','PAID'] },
                                ].map((step, i) => (
                                    <div key={i} className={`step ${step.statuses.includes(activeOrder.status?.toUpperCase()) ? 'active' : ''}`}>
                        ...
                        <div className="tracker-status-text">
                            {activeOrder.waitingWaiterAck && "🔔 Waiting for waiter to accept order..."}
                            {!activeOrder.waitingWaiterAck && activeOrder.status?.toUpperCase() === 'PENDING' && "🍳 Preparing  •  Order sent to kitchen"}
                            {!activeOrder.waitingWaiterAck && activeOrder.status?.toUpperCase() === 'PREPARING' && "🍳 Cooking  •  Takes ~15-20 mins"}
                            ...
                            {!['PENDING','PREPARING','READY','SERVED','PAID'].includes(activeOrder.status?.toUpperCase()) && `Status: ${activeOrder.status}`}
                        </div>
                        ...
                        <div className="embedded-bill-wrap">
                            ...
                            {activeOrder.status?.toUpperCase() === 'PAID' && (
                                <div style={{ marginTop: '16px' }}>
                                    <div style={{ padding: '16px', background: '#e8f5e9', borderRadius: '12px', border: '1px solid #c8e6c9', color: '#2e7d32', textAlign: 'center', marginBottom: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                                        <div style={{ fontSize: '24px' }}>✅</div>
                                        <div style={{ fontWeight: 'bold' }}>Payment Complete</div>
                                        <div style={{ fontSize: '14px', opacity: 0.9 }}>Thank you for visiting!</div>
                                    </div>
                                    
                                    {/* Feedback Section */}
                                    {(!activeOrder.rating && !feedbackSubmitted) ? (
                                        <div style={{ padding: '16px', background: 'var(--surface)', borderRadius: '12px', border: '1px solid var(--border)' }}>
                                            <h4 style={{ margin: '0 0 12px 0', textAlign: 'center' }}>How was your food?</h4>
                                                ...
                                                {[1, 2, 3, 4, 5].map((star) => (
                                                    <span 
                                                        ...
                                                        style={{ 
                                                            cursor: 'pointer', 
                                                            fontSize: '32px', 
                                                            color: (hoverRating || rating) >= star ? '#FFD700' : 'var(--border)',
                                            ...
                                            {rating > 0 && (
                                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                                                    ...
                                                    <button 
                                                        ...
                                                        style={{ width: '100%', padding: '12px', background: 'var(--brand, #C6F53D)', color: '#000', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}
                                                    >
                                                        Submit Feedback
                                                    </button>
                                                </motion.div>
                                        ...
                                        <div style={{ padding: '16px', background: 'var(--surface-2)', borderRadius: '12px', textAlign: 'center', color: 'var(--text-secondary)' }}>
                                            <div style={{ fontSize: '24px', marginBottom: '8px' }}>❤️</div>
                                            <div style={{ fontWeight: 'bold' }}>Thank you for your feedback!</div>
                                            {activeOrder.rating && (
                                                <div style={{ marginTop: '8px', color: '#FFD700', fontSize: '20px' }}>
                                                    {'★'.repeat(activeOrder.rating)}{'☆'.repeat(5 - activeOrder.rating)}
                                                </div>
                                            )}
                                        </div>
                                    )}
