...
public class DataSourceConfig {
    ...
    /**
     * Creates the local database inside the embedded PostgreSQL if it doesn't
     * exist.
     */
    private void ensureLocalDatabase() {
        ...
        try (java.sql.Connection conn = java.sql.DriverManager.getConnection(adminUrl, "postgres", "postgres");
            ...
            log.warn("[DataSource] Could not ensure local database (will retry on first connection): {}",
                    e.getMessage());
        }
    }
