...
public class ReportService {
    ...
    // PDF ENTRY POINT
    // ============================================================
    public byte[] generateReportPDF(String reportType, User restaurant, LocalDateTime start, LocalDateTime end)
            throws IOException {
    ...
    private void addReportContent(Document document, String reportType, User restaurant, LocalDateTime start,
            LocalDateTime end) {
        switch (reportType) {
            case "sales-summary":
                addSalesSummaryContent(document, restaurant, start, end);
                break;
            case "sales-report":
                addSalesReportContent(document, restaurant, start, end);
                break;
            case "sales-gst-report":
                addSalesGstReportContent(document, restaurant, start, end, true);
                break;
            case "sales-non-gst-report":
                addSalesGstReportContent(document, restaurant, start, end, false);
                break;
            case "monthly-day-wise":
                addMonthlyDayWiseContent(document, restaurant, start, end);
                break;
            case "end-day-report":
                addEndDayReportContent(document, restaurant);
                break;
            case "category-item-wise":
            case "item-wise-sales":
                addItemWiseSalesContent(document, restaurant, start, end);
                break;
            case "income-expense":
                addIncomeExpenseContent(document, restaurant, start, end);
                break;
            case "stock-report":
            case "total-inventory-valuation":
                addInventoryValuationContent(document, restaurant);
                break;
            case "month-wise-stock-report":
                addMonthWiseStockContent(document, restaurant, start, end);
                break;
            case "purchase-item-stock":
                addPurchaseReportContent(document, restaurant, start, end);
                break;
            case "cashier-wise-sales":
                addCashierSalesReportContent(document, restaurant, start, end);
                break;
            case "cancelled-item-summary":
                addCancelledReportContent(document, restaurant, start, end);
                break;
            case "gst-ledger-report":
                addLedgerReportContent(document, restaurant, start, end);
                break;
            case "expenditure-report":
                addExpenditureReportContent(document, restaurant, start, end);
                break;
            case "purchase-gst-report":
                addPurchaseGstReportContent(document, restaurant, start, end, true);
                break;
            case "purchase-non-gst-report":
                addPurchaseGstReportContent(document, restaurant, start, end, false);
                break;
            default:
                document.add(new Paragraph("Report type '" + reportType + "' is not yet available.",
                        getFont(10, false, TEXT_MUTED)));
        }
    }
    ...
    // ============================================================
    private void addProfessionalHeader(Document document, PdfWriter writer, String reportType, User restaurant,
    ...
    private void addFooter(PdfWriter writer, Document document, User restaurant) {
        try {
            ...
            ColumnText.showTextAligned(cb, Element.ALIGN_LEFT,
                    new Phrase(
                            "© " + LocalDate.now().getYear() + " " + restaurant.getRestaurantName() + " — Confidential",
                            footerFont),
    ...
    private void addSalesGstReportContent(Document document, User restaurant, LocalDateTime start, LocalDateTime end,
            boolean withGst) {
        List<Orders> all = orderRepository.findByRestaurantAndCreatedAtBetweenOrderByCreatedAtDesc(restaurant, start,
                end);

        List<Orders> filtered = all.stream()
                .filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED && o.getItems() != null
                        && (withGst ? (o.getPrintWithGst() != Boolean.FALSE && o.getItems().stream().anyMatch(item -> {
        ...
        for (Orders o : filtered) {
            ...
            if (o.getExtraCharges() != null) {
                orderExtra = o.getExtraCharges().stream().mapToDouble(com.probloom.model.entity.ExtraCharge::getAmount)
                        .sum() * proportion;
            }
    ...
    private void addSalesSummaryContent(Document document, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        List<Transaction> transactions = transactionRepository.findByRestaurantAndDateBetweenOrderByDateDesc(restaurant,
                start, end);

        double grossSales = orders.stream().filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED)
                .mapToDouble(Orders::getTotal).sum();
        double totalTax = orders.stream().filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED)
                .mapToDouble(o -> o.getTaxAmount() != null ? o.getTaxAmount() : 0.0).sum();
        double cgst = totalTax / 2;
        double sgst = totalTax / 2;
        double netSales = grossSales - totalTax;
        double totalIncome = transactions.stream().filter(t -> t.getType() == Transaction.TransactionType.INCOME)
                .mapToDouble(Transaction::getAmount).sum();
        double totalExpense = transactions.stream().filter(t -> t.getType() == Transaction.TransactionType.EXPENSE)
                .mapToDouble(Transaction::getAmount).sum();
    ...
    private void addMonthlyDayWiseContent(Document document, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        for (Orders o : orders) {
            ...
            for (OrderItem item : o.getItems()) {
                itemSales.merge(item.getName(), item.getQuantity() != null ? item.getQuantity() : 0.0, (a, b) -> a + b);
            }
        }
        ...
        for (Map.Entry<String, Double> e : dailyRevenue.entrySet()) {
            addDataRow(table, i++ % 2 == 0, e.getKey(), String.valueOf(dailyOrders.getOrDefault(e.getKey(), 0)),
        ...
        int j = 0;
        List<Map.Entry<String, Double>> sortedItems = new ArrayList<>(itemSales.entrySet());
        sortedItems.sort(Map.Entry.<String, Double>comparingByValue().reversed());
        for (Map.Entry<String, Double> e : sortedItems.stream().limit(20).toList()) {
    ...
    private void addEndDayReportContent(Document document, User restaurant) {
        ...
        double total = orders.stream().filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED)
                .mapToDouble(Orders::getTotal).sum();
        double tax = orders.stream().filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED)
                .mapToDouble(o -> o.getTaxAmount() != null ? o.getTaxAmount() : 0.0).sum();
    ...
    private void addItemWiseSalesContent(Document document, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        List<Map.Entry<String, Double>> sorted = new ArrayList<>(sales.entrySet());
        ...
        for (Map.Entry<String, Double> e : sorted) {
            addDataRow(table, i++ % 2 == 0, e.getKey(), String.valueOf(e.getValue()),
                    String.format("₹%,.2f", revenue.getOrDefault(e.getKey(), 0.0)));
        }
        document.add(table);
    }
    ...
    private void addCashierSalesReportContent(Document document, User restaurant, LocalDateTime start,
            LocalDateTime end) {
        List<Orders> orders = orderRepository.findByRestaurantAndCreatedAtBetweenOrderByCreatedAtDesc(restaurant, start,
                end);
        Map<String, Double> sales = new LinkedHashMap<>();
        ...
        for (Map.Entry<String, Double> e : sales.entrySet()) {
            addDataRow(table, i++ % 2 == 0, e.getKey(), String.valueOf(counts.get(e.getKey())),
                    String.format("₹%,.2f", e.getValue()));
        }
        document.add(table);
    }
    ...
    private void addCancelledReportContent(Document document, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        for (int i = 0; i < cancelled.size(); i++) {
            ...
            addDataRow(table, i % 2 == 0, o.getOrderNumber(), o.getCreatedAt().format(DT_FMT),
                    String.format("₹%,.2f", o.getTotal()));
        }
        document.add(table);
    }

    private void addExpenditureReportContent(Document document, User restaurant, LocalDateTime start,
            LocalDateTime end) {
        List<Transaction> transactions = transactionRepository.findByRestaurantAndDateBetweenOrderByDateDesc(restaurant,
                start, end);
        List<Transaction> expenses = transactions.stream()
                .filter(t -> t.getType() == Transaction.TransactionType.EXPENSE).toList();
    ...
    private void addPurchaseGstReportContent(Document document, User restaurant, LocalDateTime start, LocalDateTime end,
            boolean withGst) {
        List<Transaction> allExpenses = transactionRepository
    ...
    private void addInventoryValuationContent(Document document, User restaurant) {
        ...
        PdfPTable table = createStyledTable(6, 30, 12, 14, 14, 15, 15);
        addHeaderRow(table, "Item Name", "Category", "Purchased Stock", "Sold Stock", "Current Stock", "Cost/Unit (₹)");
        for (int i = 0; i < items.size(); i++) {
            InventoryItem item = items.get(i);
            double currentStock = item.getCurrentStock() != null ? item.getCurrentStock() : 0.0;
            int soldCount = soldMap.getOrDefault(item.getId(), 0);
            double purchasedCount = currentStock + soldCount;
            String unit = item.getUnit() != null ? item.getUnit().toString() : "";
            addDataRow(table, i % 2 == 0,
                    item.getName(),
                    item.getCategory() != null ? item.getCategory() : "-",
                    String.format("%.2f %s", purchasedCount, unit).trim(),
                    String.format("%.2f %s", (double) soldCount, unit).trim(),
                    String.format("%.2f %s", currentStock, unit).trim(),
                    item.getCostPerUnit() != null ? String.format("₹%.2f", item.getCostPerUnit()) : "—");
        }
        document.add(table);
    }

    private void addMonthWiseStockContent(Document document, User restaurant, LocalDateTime start, LocalDateTime end) {
        List<StockMovement> allMovements = stockMovementRepository
                .findByRestaurantAndMovementTimestampBetweenOrderByMovementTimestampDesc(restaurant, start, end);

        // month -> item -> [added, consumed]
        Map<String, Map<String, double[]>> monthItemStats = new TreeMap<>();
        for (StockMovement sm : allMovements) {
            LocalDateTime ts = sm.getTimestamp();
            if (ts == null)
                continue;
            ...
            monthItemStats.putIfAbsent(monthStr, new TreeMap<>());
            Map<String, double[]> itemStats = monthItemStats.get(monthStr);
        ...
        addSectionTitle(document, "Month Wise Stock Report");

        if (monthItemStats.isEmpty()) {
            document.add(
                    new Paragraph("No stock movements found for the selected period.", getFont(10, false, TEXT_MUTED)));
            return;
        }

        PdfPTable table = createStyledTable(5, 15, 30, 18, 18, 19);
        addHeaderRow(table, "Month", "Item Name", "Total Added", "Total Consumed", "Net Change");

        int rowIdx = 0;
        for (Map.Entry<String, Map<String, double[]>> monthEntry : monthItemStats.entrySet()) {
            for (Map.Entry<String, double[]> itemEntry : monthEntry.getValue().entrySet()) {
                double added = itemEntry.getValue()[0];
                double consumed = itemEntry.getValue()[1];
                double net = added - consumed;
                String addedStr = added > 0 ? String.format("+%.2f", added) : "Not Added";
                String consumedStr = consumed > 0 ? String.format("-%.2f", consumed) : "—";
                String netStr = net > 0 ? String.format("+%.2f", net)
                        : net < 0 ? String.format("%.2f", net) : "0.00";
                addDataRow(table, rowIdx++ % 2 == 0,
                        monthEntry.getKey(),
                        itemEntry.getKey(),
                        addedStr,
                        consumedStr,
    ...
    private static class LedgerEntry {
        ...
        public LedgerEntry(LocalDateTime d, String cr, String p, String vt, String vn, double dr, double crAmount) {
            ...
            this.particulars = p;
            this.voucherType = vt;
            this.voucherNumber = vn;
            this.debit = dr;
            this.credit = crAmount;
        }
    ...
    private List<LedgerEntry> getLedgerEntries(User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        for (Orders o : orders) {
            ...
            entries.add(new LedgerEntry(
                    o.getCreatedAt(),
                    "Cr",
                    o.getCustomerName() != null && !o.getCustomerName().isBlank() ? o.getCustomerName()
                            : (o.getOrderType() != null ? o.getOrderType().name() : "Walk-in"),
                    "Sale",
                    o.getOrderNumber(),
                    0.0,
                    o.getTotal()));
    ...
    private void addLedgerReportContent(Document document, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        addHeaderRow(table, "Date", "Cr/Dr", "Particulars", "Voucher Type", "Voucher Number", "Debit (₹)",
                "Credit (₹)");
        ...
        PdfPCell creditCell = new PdfPCell(
                new Phrase(String.format("%,.2f", totalCredit), getFont(9, true, BRAND_DARK)));
    ...
    // ============================================================
    public byte[] generateReportWord(String reportType, User restaurant, LocalDateTime start, LocalDateTime end)
            throws IOException {
    ...
    private void addWordHeader(XWPFDocument doc, String reportType, User restaurant, LocalDateTime start,
            LocalDateTime end) {
        // Restaurant name
        ...
        if (restaurant.getAddress() != null || restaurant.getPhone() != null) {
            ...
            if (restaurant.getPhone() != null)
                sb.append("  |  Tel: ").append(restaurant.getPhone());
            if (restaurant.getEmail() != null)
                sb.append("  |  ").append(restaurant.getEmail());
        ...
        if (start != null && end != null) {
            ...
            pr.setText("Period: " + start.format(DATE_FMT) + " → " + end.format(DATE_FMT) + "   |   Generated: "
    ...
    private void addWordContent(XWPFDocument doc, String reportType, User restaurant, LocalDateTime start,
            LocalDateTime end) {
        switch (reportType) {
            case "sales-summary":
                buildWordSalesSummary(doc, restaurant, start, end);
                break;
            case "sales-report":
                buildWordSalesReport(doc, restaurant, start, end);
                break;
            case "sales-gst-report":
                buildWordSalesGstReport(doc, restaurant, start, end, true);
                break;
            case "sales-non-gst-report":
                buildWordSalesGstReport(doc, restaurant, start, end, false);
                break;
            case "item-wise-sales":
            case "category-item-wise":
                buildWordItemWise(doc, restaurant, start, end);
                break;
            case "cashier-wise-sales":
                buildWordCashierReport(doc, restaurant, start, end);
                break;
            case "cancelled-item-summary":
                buildWordCancelledReport(doc, restaurant, start, end);
                break;
            case "income-expense":
                buildWordIncomeExpense(doc, restaurant, start, end);
                break;
            case "stock-report":
            case "total-inventory-valuation":
                buildWordInventory(doc, restaurant);
                break;
            case "month-wise-stock-report":
                buildWordMonthWiseStock(doc, restaurant, start, end);
                break;
            case "gst-ledger-report":
                buildWordLedgerReport(doc, restaurant, start, end);
                break;
            case "expenditure-report":
                buildWordExpenditureReport(doc, restaurant, start, end);
                break;
            case "purchase-gst-report":
                buildWordPurchaseGstReport(doc, restaurant, start, end, true);
                break;
            case "purchase-non-gst-report":
                buildWordPurchaseGstReport(doc, restaurant, start, end, false);
                break;
            default:
    ...
    private void buildWordSalesSummary(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        double gross = orders.stream().filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED)
                .mapToDouble(Orders::getTotal).sum();
        double tax = orders.stream().filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED)
                .mapToDouble(o -> o.getTaxAmount() != null ? o.getTaxAmount() : 0.0).sum();
    ...
    private void buildWordSalesGstReport(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end,
            boolean withGst) {
        List<Orders> all = orderRepository.findByRestaurantAndCreatedAtBetweenOrderByCreatedAtDesc(restaurant, start,
                end);
        ...
        if (withGst)
            summary += "   |   Base: " + String.format("₹%,.2f", baseRevenue) + "   |   GST: "
    ...
    private void buildWordItemWise(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        sorted.sort(Map.Entry.<String, Double>comparingByValue().reversed());
        String[][] data = sorted.stream().map(e -> new String[] { e.getKey(), String.valueOf(e.getValue()),
                String.format("₹%,.2f", revMap.get(e.getKey())) }).toArray(String[][]::new);
    ...
    private void buildWordCashierReport(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        String[][] data = salesMap.entrySet().stream()
                .map(e -> new String[] { e.getKey(), String.format("₹%,.2f", e.getValue()) }).toArray(String[][]::new);
    ...
    private void buildWordPurchaseGstReport(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end,
            boolean withGst) {
        ...
        if (withGst)
            summary += "   |   Base: " + String.format("₹%,.2f", baseAmount) + "   |   GST: "
    ...
    private void buildWordCancelledReport(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        List<Orders> cancelled = orders.stream().filter(o -> o.getStatus() == Orders.OrderStatus.CANCELLED).toList();
        String[][] data = cancelled.stream().map(o -> new String[] { o.getOrderNumber(),
                o.getCreatedAt().format(DT_FMT), String.format("₹%,.2f", o.getTotal()) }).toArray(String[][]::new);
    ...
    private void buildWordIncomeExpense(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end) {
        List<Transaction> transactions = transactionRepository.findByRestaurantAndDateBetweenOrderByDateDesc(restaurant,
                start, end);
        String[][] data = transactions.stream().map(t -> new String[] { t.getDate().format(DATE_FMT),
                t.getDescription(), t.getType().toString(), String.format("₹%,.2f", t.getAmount()) })
                .toArray(String[][]::new);
    ...
    private void buildWordInventory(XWPFDocument doc, User restaurant) {
        ...
        String[][] data = items.stream().map(i -> {
            ...
            double purchasedCount = currentStock + soldCount;
            String unit = i.getUnit() != null ? i.getUnit().toString() : "";
            return new String[] {
                    i.getName(),
                    ...
                    String.format("%.2f %s", (double) soldCount, unit).trim(),
                    String.format("%.2f %s", currentStock, unit).trim(),
                    i.getCostPerUnit() != null ? String.format("₹%.2f", i.getCostPerUnit()) : "—"
            };
        }).toArray(String[][]::new);
        buildWordTable(doc,
                new String[] { "Item Name", "Category", "Purchased Stock", "Sold Stock", "Current Stock", "Cost/Unit" },
                data);
    }

    private void buildWordMonthWiseStock(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end) {
        List<StockMovement> allMovements = stockMovementRepository
                .findByRestaurantAndMovementTimestampBetweenOrderByMovementTimestampDesc(restaurant, start, end);
        Map<String, Map<String, double[]>> monthItemStats = new TreeMap<>();
        for (StockMovement sm : allMovements) {
        ...
        for (Map.Entry<String, Map<String, double[]>> monthEntry : monthItemStats.entrySet()) {
            for (Map.Entry<String, double[]> itemEntry : monthEntry.getValue().entrySet()) {
                ...
                rows.add(new String[] {
                        monthEntry.getKey(),
                        itemEntry.getKey(),
                        added > 0 ? String.format("+%.2f", added) : "Not Added",
                        consumed > 0 ? String.format("-%.2f", consumed) : "—",
                        net > 0 ? String.format("+%.2f", net) : net < 0 ? String.format("%.2f", net) : "0.00"
                });
            }
        }
        buildWordTable(doc, new String[] { "Month", "Item Name", "Total Added", "Total Consumed", "Net Change" },
    ...
    private void buildWordLedgerReport(XWPFDocument doc, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        data[entries.size()] = new String[] { "", "", "", "", "Closing Balance", String.format("%,.2f", totalDebit),
                String.format("%,.2f", totalCredit) };
        buildWordTable(doc,
                new String[] { "Date", "Cr/Dr", "Particulars", "Voucher Type", "Voucher Number", "Debit", "Credit" },
                data);
    }
    ...
    // JSON GENERATION
    // ============================================================
    public byte[] generateReportJson(String reportType, User restaurant, LocalDateTime start, LocalDateTime end)
            throws IOException {
        ...
        if (start != null)
            payload.put("periodFrom", start.format(DATE_FMT));
        if (end != null)
            payload.put("periodTo", end.format(DATE_FMT));
        payload.put("generatedAt", LocalDateTime.now().format(DT_FMT));

        switch (reportType) {
            case "sales-summary":
                buildJsonSalesSummary(payload, restaurant, start, end);
                break;
            case "sales-report":
                buildJsonSalesReport(payload, restaurant, start, end);
                break;
            case "item-wise-sales":
            case "category-item-wise":
                buildJsonItemWise(payload, restaurant, start, end);
                break;
            case "cashier-wise-sales":
                buildJsonCashierReport(payload, restaurant, start, end);
                break;
            case "income-expense":
                buildJsonIncomeExpense(payload, restaurant, start, end);
                break;
            case "stock-report":
            case "total-inventory-valuation":
                buildJsonInventory(payload, restaurant);
                break;
            case "cancelled-item-summary":
                buildJsonCancelled(payload, restaurant, start, end);
                break;
            case "gst-ledger-report":
                buildJsonLedgerReport(payload, restaurant, start, end);
                break;
            default:
    ...
    private void buildJsonSalesReport(Map<String, Object> p, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        p.put("orders", list);
        p.put("totalRevenue", orders.stream().filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED)
                .mapToDouble(Orders::getTotal).sum());
    }
    ...
    private void buildJsonItemWise(Map<String, Object> p, User restaurant, LocalDateTime start, LocalDateTime end) {
        ...
        salesMap.forEach((name, qty) -> {
            ...
            m.put("revenue", revMap.get(name));
            list.add(m);
        });
        list.sort((a, b) -> Double.compare((Double) b.get("quantitySold"), (Double) a.get("quantitySold")));
        p.put("items", list);
    }

    private void buildJsonCashierReport(Map<String, Object> p, User restaurant, LocalDateTime start,
            LocalDateTime end) {
        List<Orders> orders = orderRepository.findByRestaurantAndCreatedAtBetweenOrderByCreatedAtDesc(restaurant, start,
                end);
        Map<String, Double> s = new LinkedHashMap<>();
        orders.stream().filter(o -> o.getStatus() != Orders.OrderStatus.CANCELLED)
                .forEach(o -> s.merge(o.getCreatedBy() != null ? o.getCreatedBy().getName() : "System", o.getTotal(),
                        (a, b) -> a + b));
        p.put("cashierSales", s);
    }

    private void buildJsonIncomeExpense(Map<String, Object> p, User restaurant, LocalDateTime start,
            LocalDateTime end) {
        List<Transaction> tx = transactionRepository.findByRestaurantAndDateBetweenOrderByDateDesc(restaurant, start,
                end);
        p.put("transactions", tx);
        p.put("totalIncome", tx.stream().filter(t -> t.getType() == Transaction.TransactionType.INCOME)
                .mapToDouble(Transaction::getAmount).sum());
        p.put("totalExpense", tx.stream().filter(t -> t.getType() == Transaction.TransactionType.EXPENSE)
                .mapToDouble(Transaction::getAmount).sum());
    }

    private void buildJsonInventory(Map<String, Object> p, User restaurant) {
        List<InventoryItem> items = inventoryItemRepository.findByRestaurantAndIsActiveTrueOrderByNameAsc(restaurant);
        double totalVal = items.stream().mapToDouble(i -> (i.getCurrentStock() != null ? i.getCurrentStock() : 0.0)
                * (i.getCostPerUnit() != null ? i.getCostPerUnit() : 0.0)).sum();
        p.put("items", items);
