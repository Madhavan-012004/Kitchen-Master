...
public class PrintService {
    ...
    public boolean printKOT(Orders order, String printerIp) {
        ...
        try (Socket socket = new Socket(printerIp.trim(), 9100);
            ...
            write(out, "Order: " + (order.getOrderNumber() != null ? order.getOrderNumber()
            ...
            if (order.getItems() != null) {
                for (OrderItem item : order.getItems()) {
                    ...
                    totalItems += (int) Math.ceil(itemQty);
                    String name = item.getName();
                    if (name.length() > 25)
                        name = name.substring(0, 25);
                    // Format quantity: show as integer if whole number, decimal otherwise
                    String qtyStr = (itemQty == Math.floor(itemQty)) ? String.valueOf((int) itemQty)
                            : String.format("%.3f", itemQty);
                    write(out, String.format("%-25s %5s\n", name, qtyStr));
    ...
    public boolean printBill(Orders order, User restaurant, String printerIp) {
        ...
        try (Socket socket = new Socket(printerIp.trim(), 9100);
            ...
            boolean printGst = Boolean.TRUE.equals(order.getPrintWithGst())
                    || (order.getPrintWithGst() == null && order.getTaxAmount() != null && order.getTaxAmount() > 0.0);
            ...
            // Meta
            write(out, ALIGN_LEFT);
            write(out, "Bill No : " + (order.getOrderNumber() != null ? order.getOrderNumber()
                    : order.getId().toString().substring(0, 8)) + "\n");

            boolean isRestaurant = restaurant.getPreferredPosMode() == null
                    || restaurant.getPreferredPosMode().equals("restaurant");
            ...
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm");
            write(out, "Date    : " + order.getCreatedAt().format(formatter) + "\n");
            if (isRestaurant && order.getWaiterName() != null) {
            ...
            if (order.getItems() != null) {
                for (OrderItem item : order.getItems()) {
                    ...
                    write(out, String.format("%-6s %-12s %5s %8.2f\n", cat, name, qtyStr, tot));
                }
            }
            write(out, "--------------------------------\n");
            ...
            write(out, ALIGN_CENTER);
            write(out, "Payment: "
                    + (order.getPaymentMethod() != null ? order.getPaymentMethod().name().toUpperCase() : "CASH")
                    + "\n");
