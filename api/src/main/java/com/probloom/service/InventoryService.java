...
public class InventoryService {
    ...
    public InventoryItem getByName(@NonNull User restaurant, @NonNull String name) {
        List<InventoryItem> items = inventoryItemRepository.findByRestaurantAndNameIgnoreCaseAndIsActiveTrue(restaurant,
                name);
        if (items.isEmpty()) {
    ...
    public InventoryItem create(@NonNull User restaurant, @NonNull Map<String, Object> data) {
        ...
        InventoryItem item = InventoryItem.builder()
                ...
                .price(data.containsKey("price") && data.get("price") != null
                        ? safeParseDouble(data.get("price").toString())
                        : 0.0)
                .isBilliable(!data.containsKey("isBilliable") || data.get("isBilliable") == null
                        || Boolean.valueOf(data.get("isBilliable").toString()))
                ...
                .currentStock(data.containsKey("currentStock") && data.get("currentStock") != null
                        ? safeParseDouble(data.get("currentStock").toString())
                        : 0.0)
                .lowStockThreshold(data.containsKey("lowStockThreshold") && data.get("lowStockThreshold") != null
                        ? safeParseDouble(data.get("lowStockThreshold").toString())
                        : 1.0)
                .costPerUnit(data.containsKey("costPerUnit") && data.get("costPerUnit") != null
                        ? safeParseDouble(data.get("costPerUnit").toString())
                        : 0.0)
                ...
                .packMultiplier(data.containsKey("packMultiplier")
                        ? (int) safeParseDouble(data.get("packMultiplier").toString())
                        : 1)
                .gstPercent(data.containsKey("gstPercent") && data.get("gstPercent") != null
                        ? safeParseDouble(data.get("gstPercent").toString())
                        : 0.0)
        ...
        // Clothing ERP fields (set after build since builder doesn't have them)
        if (data.containsKey("brand"))
            item.setBrand((String) data.get("brand"));
        if (data.containsKey("subCategory"))
            item.setSubCategory((String) data.get("subCategory"));
        if (data.containsKey("itemType"))
            item.setItemType((String) data.get("itemType"));
        if (data.containsKey("purchaseInvoiceNo"))
            item.setPurchaseInvoiceNo((String) data.get("purchaseInvoiceNo"));
        if (data.containsKey("purchaseDate"))
            item.setPurchaseDate((String) data.get("purchaseDate"));
        if (data.containsKey("storageLocation"))
            item.setStorageLocation((String) data.get("storageLocation"));
        if (data.containsKey("fabricType"))
            item.setFabricType((String) data.get("fabricType"));
        if (data.containsKey("color"))
            item.setColor((String) data.get("color"));
        if (data.containsKey("pattern"))
            item.setPattern((String) data.get("pattern"));
        if (data.containsKey("widthInches"))
            item.setWidthInches((String) data.get("widthInches"));
        if (data.containsKey("gsm"))
            item.setGsm((String) data.get("gsm"));
        if (data.containsKey("materialComposition"))
            item.setMaterialComposition((String) data.get("materialComposition"));
        if (data.containsKey("rollNumber"))
            item.setRollNumber((String) data.get("rollNumber"));
        if (data.containsKey("gender"))
            item.setGender((String) data.get("gender"));
        if (data.containsKey("size"))
            item.setSize((String) data.get("size"));
        if (data.containsKey("fitType"))
            item.setFitType((String) data.get("fitType"));
        if (data.containsKey("season"))
            item.setSeason((String) data.get("season"));
        if (data.containsKey("discountPercent") && data.get("discountPercent") != null)
            item.setDiscountPercent(safeParseDouble(data.get("discountPercent").toString()));
        if (data.containsKey("openingStock") && data.get("openingStock") != null)
            item.setOpeningStock(safeParseDouble(data.get("openingStock").toString()));
        if (data.containsKey("damagedQty") && data.get("damagedQty") != null)
            item.setDamagedQty(safeParseDouble(data.get("damagedQty").toString()));
        if (data.containsKey("returnedQty") && data.get("returnedQty") != null)
            item.setReturnedQty(safeParseDouble(data.get("returnedQty").toString()));
        if (data.containsKey("reservedQty") && data.get("reservedQty") != null)
            item.setReservedQty(safeParseDouble(data.get("reservedQty").toString()));
        if (item.getOpeningStock() == null)
            item.setOpeningStock(item.getCurrentStock());

        InventoryItem savedItem = inventoryItemRepository.save(java.util.Objects.requireNonNull(item));

        // Record Initial Expense if cost provided (only for paid quantity, not free
        // packs)
        ...
        if (recordExpense && savedItem.getCostPerUnit() > 0 && savedItem.getCurrentStock() > 0) {
            ...
            if (data.containsKey("freePieces")) {
                paidPieces -= safeParseDouble(data.get("freePieces").toString());
            } else if (data.containsKey("freeStock")) {
                paidPieces -= safeParseDouble(data.get("freeStock").toString());
            }
            if (paidPieces > 0) {
    ...
    public InventoryItem update(@NonNull User restaurant, @NonNull Long id, @NonNull Map<String, Object> data) {
        ...
        if (data.containsKey("price"))
            item.setPrice(safeParseDouble(data.get("price").toString()));
        if (data.containsKey("isBilliable"))
            item.setIsBilliable(Boolean.valueOf(data.get("isBilliable").toString()));

        if (data.containsKey("category"))
            item.setCategory((String) data.get("category"));
        if (data.containsKey("unit"))
            item.setUnit(InventoryItem.Unit.valueOf(data.get("unit").toString().toUpperCase()));
        if (data.containsKey("lowStockThreshold"))
            item.setLowStockThreshold(safeParseDouble(data.get("lowStockThreshold").toString()));
        if (data.containsKey("costPerUnit"))
            item.setCostPerUnit(safeParseDouble(data.get("costPerUnit").toString()));
        if (data.containsKey("supplierName"))
            item.setSupplierName((String) data.get("supplierName"));
        if (data.containsKey("supplierPhone"))
            item.setSupplierPhone((String) data.get("supplierPhone"));
        if (data.containsKey("packMultiplier") && data.get("packMultiplier") != null)
            item.setPackMultiplier((int) safeParseDouble(data.get("packMultiplier").toString()));
        if (data.containsKey("manufacturer"))
            item.setManufacturer((String) data.get("manufacturer"));
        if (data.containsKey("batchNo"))
            item.setBatchNo((String) data.get("batchNo"));
        if (data.containsKey("expDate"))
            item.setExpDate((String) data.get("expDate"));
        if (data.containsKey("packSize"))
            item.setPackSize((String) data.get("packSize"));
        if (data.containsKey("hsnCode"))
            item.setHsnCode((String) data.get("hsnCode"));
        if (data.containsKey("gstPercent") && data.get("gstPercent") != null)
            item.setGstPercent(safeParseDouble(data.get("gstPercent").toString()));
    ...
    @Transactional
    @NonNull
    public InventoryItem adjustStock(@NonNull User restaurant, @NonNull Long id, @NonNull Map<String, Object> data,
            User performedBy) {
        ...
        switch (movementType) {
            case ADD:
                ...
                System.out.println("➕ Stock added: " + item.getName() + " (+" + quantity + " paid, +" + freeQuantity
                        + " free) New stock: " + item.getCurrentStock());
                break;
            case DEDUCT:
                // Do not allow negative stock
                item.setCurrentStock(Math.max(0.0, item.getCurrentStock() - quantity));
                System.out.println("➖ Stock deducted: " + item.getName() + " (-" + quantity + ") New stock: "
        ...
        // Record Expenditure if cost provided OR automatically calculate it for ADD
        // movement
        if (movementType == StockMovement.MovementType.ADD) {
            ...
            if (totalCost != null && totalCost > 0) {
                ...
                if (data.containsKey("gstAmount"))
                    gstAmount = Double.valueOf(data.get("gstAmount").toString());
                if (data.containsKey("discountAmount"))
                    discountAmount = Double.valueOf(data.get("discountAmount").toString());
    ...
    @Transactional
    @NonNull
    public InventoryItem incrementStockByBarcode(@NonNull User restaurant, @NonNull String barcode, double amount,
            User performedBy) {
    ...
    public List<Map<String, Object>> parseInvoiceCsv(byte[] fileData) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(
                new java.io.ByteArrayInputStream(fileData), java.nio.charset.StandardCharsets.UTF_8))) {
            String line;
            String[] headers = null;
     ...
     * Rows sharing the same invoice number are grouped into ONE expenditure
     * transaction.
     */
    @Transactional
    public int importCsv(User restaurant, MultipartFile file) throws Exception {
        ...
        // Invoice-grouped expenditure accumulator: invoiceNo -> {baseAmount, gstAmount,
        // discountAmount, itemCount, description, paymentMethod}
        Map<String, double[]> invoiceAccumulator = new LinkedHashMap<>(); // [baseAmt, gstAmt, discountAmt, itemCount,
                                                                          // totalAmt]
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                for (int i = 0; i < headers.length && i < parts.length; i++) {
                    ...
                    } else if (header.equals("ITEM DESCRIPTION") || header.equals("NAME")
                            || header.equals("DESCRIPTION")) {
                        data.put("name", val);
                    ...
                    } else if (header.equals("DISCOUNT") || header.equals("DISC") || header.equals("DISC AMT")
                            || header.equals("DISCOUT")) {
                    ...
                    } else if (header.equals("INVOICE NUMBER") || header.equals("INV NO")
                            || header.equals("INVOICE NO")) {
                        invoiceNo = val;
                    } else {
                ...
                if (data.containsKey("costPerUnit")) {
                    ratePerPack = safeParseDouble(data.get("costPerUnit").toString());
                    data.put("costPerUnit", multiplier > 0 ? ratePerPack / multiplier : ratePerPack);
                }
                if (data.containsKey("price")) {
                    double mrpPerPack = safeParseDouble(data.get("price").toString());
                    data.put("price", multiplier > 0 ? mrpPerPack / multiplier : mrpPerPack);
                }

                // ── EXPENDITURE for this line item (paid packs only) ──
                double lineBase = data.containsKey("amountFile") ? safeParseDouble(data.get("amountFile").toString())
                        : (ratePerPack * qty);
                double lineGst = data.containsKey("gstAmountFile")
                        ? safeParseDouble(data.get("gstAmountFile").toString())
                        : (lineBase * gstPct / 100.0);

                double lineTotalAmount = data.containsKey("totalAmountFile")
                        ? safeParseDouble(data.get("totalAmountFile").toString())
                        : (lineBase + lineGst - discountAmt);
                ...
                // Determine if this row has an invoice number
                boolean hasInvoice = invoiceNo != null && !invoiceNo.isEmpty();

                // Accumulate into invoice bucket (only for rows WITH an invoice number)
                if (hasInvoice && lineTotalAmount > 0) {
                ...
                List<InventoryItem> existingList = (batch != null && !batch.isEmpty())
                        ? inventoryItemRepository.findByRestaurantAndNameIgnoreCaseAndBatchNoAndIsActiveTrue(restaurant,
                                itemName, batch)
                        : inventoryItemRepository.findByRestaurantAndNameIgnoreCaseAndIsActiveTrue(restaurant,
                                itemName);

                double incomingPrice = data.containsKey("price") ? safeParseDouble(data.get("price").toString()) : 0.0;

                java.util.Optional<InventoryItem> existing = existingList.stream()
                        .filter(item -> Math
                                .abs((item.getPrice() != null ? item.getPrice() : 0.0) - incomingPrice) < 0.001)
                        .findFirst();
                ...
                if (existing.isPresent()) {
                    ...
                    // ── Record expenditure for existing items WITHOUT invoice number ──
                    // Items WITH invoice are handled by the invoice accumulator after the loop
                    if (!hasInvoice && lineTotalAmount > 0) {
                        transactionService.create(restaurant, java.util.Objects.requireNonNull(java.util.Map.of(
                                "amount", lineTotalAmount,
                                "category", "Inventory Purchase",
                                "description",
                                "CSV Import - Restock: " + item.getName() + " (" + (int) paidPieces + " pcs)",
                                "paymentMethod", "Cash",
                                "gstAmount", lineGst,
                    ...
                    // Create new item
                    if (hasInvoice) {
                        // Invoice items: expense recorded via invoice accumulator after the loop
                        data.put("recordExpense", false);
                    } else {
                        // No invoice: let create() record expense using costPerUnit × paidPieces
                        data.put("recordExpense", true);
                    }
    ...
    public InventoryItem createWithReason(User restaurant, Map<String, Object> data, String reason) {
        InventoryItem item = create(java.util.Objects.requireNonNull(restaurant),
                java.util.Objects.requireNonNull(data));
        // Find the last movement and update reason (since create() makes one)
        List<StockMovement> movements = stockMovementRepository
                .findByRestaurantOrderByMovementTimestampDesc(restaurant);
    ...
    public void bulkAdd(@NonNull User restaurant, @NonNull Map<String, Object> body) {
        ...
        for (Map<String, Object> itemData : itemsList) {
            ...
            if (existing.isPresent()) {
                ...
                if (!supplierName.isEmpty())
                    createData.put("supplierName", supplierName);
        ...
        if (totalAmount > 0 && !invoiceNo.isEmpty()) {
            ...
            if (!supplierName.isEmpty())
                finalDesc = supplierName + " - " + finalDesc;
