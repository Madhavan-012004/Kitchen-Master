...
public class OrderService {
    ...
    public List<Orders> getFilteredHistory(@NonNull User restaurant, String date, String status, String orderType, int limit) {
        ...
        if (date != null && !date.isEmpty()) {
            try {
                ...
                LocalDateTime from = localDate.atStartOfDay(ist).withZoneSameInstant(java.time.ZoneOffset.UTC).toLocalDateTime();
                LocalDateTime to = localDate.atTime(23, 59, 59).atZone(ist).withZoneSameInstant(java.time.ZoneOffset.UTC).toLocalDateTime();
                ...
                if (all.isEmpty()) {
                    all = orderRepository.findByRestaurantAndCreatedAtBetweenOrderByCreatedAtDesc(restaurant, naiveFrom, naiveTo);
                }
            } catch (Exception e) {
    ...
    public Orders create(@NonNull User restaurant, User createdBy, @NonNull Map<String, Object> data) {
        ...
        String tableNumber = normalizeTableNumber(data.get("tableNumber") != null ? data.get("tableNumber").toString() : "Takeaway");
        ...
        if (isDineIn) { // Only check for active orders if it's a dine-in order
            List<Orders> activeOrders = orderRepository.findByRestaurantAndTableNumberAndStatusNotInOrderByCreatedAtDesc(
                restaurant, tableNumber, Arrays.asList(Orders.OrderStatus.PAID, Orders.OrderStatus.CANCELLED)
            );
            if (!activeOrders.isEmpty()) {
                // If there's an active order for the same table that isn't paid, merge into it
                order = activeOrders.get(0);
                // If the existing order was cancelled, we shouldn't merge into it, but the query excludes PAID. 
        ...
        if (order == null) {
            ...
            order = Orders.builder()
                    ...
                    .discountType(data.containsKey("discountType") ? Orders.DiscountType.valueOf(data.get("discountType").toString().toUpperCase()) : Orders.DiscountType.NONE)
            ...
            // If the order was READY or SERVED, moving back to PREPARING because new items came
        ...
        if (Boolean.TRUE.equals(restaurant.getKotPrinterEnabled()) && restaurant.getKitchenPrinterIp() != null && !restaurant.getKitchenPrinterIp().trim().isEmpty()) {
    ...
    public Orders update(@NonNull User restaurant, Long id, @NonNull Map<String, Object> data) {
        ...
        if (itemsData != null) {
            ...
            // Since frontend sends all items, we can clear and re-add for simplicity if performance allows,
            ...
            for (Map<String, Object> itemData : itemsData) {
                ...
                if (oi == null) {
                    ...
                    if (itemData.get("menuItemId") != null) {
                        Long mId = Long.valueOf(itemData.get("menuItemId").toString());
                        menuItem = menuItemRepository.findById(Objects.requireNonNull(mId)).orElse(null); //NOSONAR nullable by design
                    }
        ...
        // If an order was marked READY/SERVED but now has PREPARING items (new ones added),
        // revert the order status so it reappears in the kitchen display.
        boolean hasActiveItems = order.getItems().stream()
                .anyMatch(i -> i.getStatus() == OrderItem.ItemStatus.PENDING || i.getStatus() == OrderItem.ItemStatus.PREPARING);
        ...
        if (hasActiveItems && (order.getStatus() == Orders.OrderStatus.READY || order.getStatus() == Orders.OrderStatus.SERVED)) {
            order.setStatus(Orders.OrderStatus.PREPARING);
        }
    ...
    private Double parseOptionalDouble(Map<String, Object> data, String key, Double defaultValue) {
        if (!data.containsKey(key) || data.get(key) == null) return defaultValue;
        try {
    ...
    public Orders updateStatus(@NonNull User restaurant, Long id, String status) {
        ...
        try {
            ...
            for (String table : allTables) {
                socketService.broadcastStatusUpdate(restaurant.getId(), saved.getId(), saved.getOrderNumber(), status.toUpperCase(), table);
            }
        } catch (Exception e) {
            System.err.println("⚠️ Socket broadcast failed: " + e.getMessage());
        }
    ...
    public Orders updatePayment(@NonNull User restaurant, Long id, String paymentMethod, String paymentStatus) {
        Orders order = getById(restaurant, id);
        if (paymentMethod != null) order.setPaymentMethod(Orders.PaymentMethod.valueOf(paymentMethod.toUpperCase()));
        if (paymentStatus != null) order.setPaymentStatus(Orders.PaymentStatus.valueOf(paymentStatus.toUpperCase()));
        ...
        try {
            socketService.broadcastStatusUpdate(restaurant.getId(), saved.getId(), saved.getOrderNumber(), saved.getStatus().name(), saved.getTableNumber());
        } catch (Exception e) {
            System.err.println("⚠️ Socket broadcast failed: " + e.getMessage());
        }
        return saved;
    ...
    public Orders requestBill(@NonNull User restaurant, Long id) {
        ...
        // --- SILENT IP PRINTING (BILL) ---
        if (Boolean.TRUE.equals(restaurant.getBillPrinterEnabled()) && restaurant.getCounterPrinterIp() != null && !restaurant.getCounterPrinterIp().trim().isEmpty()) {
            final Orders finalSaved = saved;
    ...
    public Orders requestBillPublic(@NonNull Long id) {
        ...
        if (Boolean.TRUE.equals(restaurantRef.getBillPrinterEnabled()) && restaurantRef.getCounterPrinterIp() != null && !restaurantRef.getCounterPrinterIp().trim().isEmpty()) {
            final Orders finalSaved = saved;
            new Thread(() -> printService.printBill(finalSaved, restaurantRef, restaurantRef.getCounterPrinterIp())).start();
        }
    ...
    public Orders updateItemStatus(@NonNull User restaurant, Long orderId, Long itemId, String status) {
        ...
        boolean allReady = order.getItems().stream()
                .allMatch(i -> i.getStatus() == OrderItem.ItemStatus.READY || i.getStatus() == OrderItem.ItemStatus.SERVED || i.getStatus() == OrderItem.ItemStatus.CANCELLED);
        ...
        try {
            socketService.broadcastItemStatusUpdate(restaurant.getId(), orderId, itemId, status, saved.getStatus().name(), saved.getTableNumber());
        } catch (Exception e) {
            System.err.println("⚠️ Socket broadcast failed: " + e.getMessage());
        }
    ...
    public Orders markItemReadyPartial(@NonNull User restaurant, Long orderId, Long itemId) {
        ...
        if (item.getCompletedQuantity() == null) item.setCompletedQuantity(0);

        if (item.getCompletedQuantity() < item.getQuantity()) {
            item.setCompletedQuantity(item.getCompletedQuantity() + 1);
        }
        ...
        boolean allReady = order.getItems().stream()
                .allMatch(i -> i.getStatus() == OrderItem.ItemStatus.READY || i.getStatus() == OrderItem.ItemStatus.SERVED);
        ...
        try {
            ...
            socketService.broadcastItemsReady(restaurant.getId(), saved.getId(), waiterId, saved.getOrderNumber(), saved.getTableNumber(), "1x " + item.getName());
        } catch (Exception e) {
            System.err.println("⚠️ Socket broadcast failed: " + e.getMessage());
        }
        ...
        if (changed) {
            ...
            try {
                ...
                if (!readyNames.isEmpty()) {
                    ...
                    socketService.broadcastItemsReady(restaurant.getId(), saved.getId(), waiterId, saved.getOrderNumber(), saved.getTableNumber(), itemsText);
                }
            } catch (Exception e) {
                System.err.println("⚠️ Socket broadcast failed: " + e.getMessage());
            }
    ...
    public Orders split(@NonNull User restaurant, Long orderId, List<Long> itemIds, String newTableNumber) {
        ...
        // Important: use index-based or iterator removal to avoid ConcurrentModificationException if needed,
        ...
        try {
            ...
            if (originalOrder.getItems().isEmpty()) {
                socketService.broadcastStatusUpdate(restaurant.getId(), originalOrder.getId(), originalOrder.getOrderNumber(), "CANCELLED", originalOrder.getTableNumber());
            } else {
    ...
    @Transactional
    public Orders combineTables(@NonNull User restaurant, String sourceTable, Long targetOrderId, String targetTable, Integer covers) {
    ...
    @Transactional
    public Orders combine(@NonNull User restaurant, Long sourceOrderId, Long targetOrderId, String targetTable, Integer covers) {
        ...
        try {
            socketService.broadcastStatusUpdate(restaurant.getId(), sourceOrder.getId(), sourceOrder.getOrderNumber(), "CANCELLED", sourceOrder.getTableNumber());
    ...
    private String generateOrderNumber(User restaurant) {
        Long maxSeq = orderRepository.findMaxOrderNumberSeq(restaurant.getId());
        long nextSeq = (maxSeq == null ? 0 : maxSeq) + 1;
        return String.format("ORD%04d", nextSeq);
    }
    ...
    public Orders uncombineTable(@NonNull Long orderId, String tableToUnmerge) {
        ...
        try {
            ...
            socketService.broadcastStatusUpdate(order.getRestaurant().getId(), saved.getId(), saved.getOrderNumber(), saved.getStatus().toString(), saved.getTableNumber());
            
            // Specifically notify that the unmerged table is now "CANCELLED" (free) to trigger a refresh on that specific UI tab
            socketService.broadcastStatusUpdate(order.getRestaurant().getId(), saved.getId(), saved.getOrderNumber(), "CANCELLED", tableToUnmerge);

            if (saved.getStatus() == Orders.OrderStatus.CANCELLED) {
                socketService.broadcastStatusUpdate(order.getRestaurant().getId(), saved.getId(), saved.getOrderNumber(), "CANCELLED", saved.getTableNumber());
            }
        } catch (Exception e) {
