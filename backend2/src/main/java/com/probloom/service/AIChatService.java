...
public class AIChatService {
        ...
        try {
            switch (parsed.intent) {
                case EMPLOYEE_PERFORMANCE:
                    return getEmployeePerformanceResponse(restaurant, parsed.startDate, parsed.endDate, parsed.periodLabel);
    ...
    private Map<String, Object> getEmployeePerformanceResponse(User restaurant, LocalDateTime start, LocalDateTime end, String periodLabel) {
        ...
        for (Orders o : orders) {
            ...
            String empName = (o.getCreatedBy() != null && o.getCreatedBy().getName() != null) ? o.getCreatedBy().getName() : "System";
        ...
        if (revenueByEmployee.isEmpty()) {
            return Map.of("answer", "I could not find any employee performance data for " + periodLabel + ".", "type", "text");
        }
        ...
        sb.append("The top performing employee is **").append(bestEmployee).append("** with a total generated revenue of **₹").append(String.format("%.2f", maxRev)).append("**.\n\n");
        ...
        revenueByEmployee.entrySet().stream()
            ...
            .forEach(e -> {
                sb.append("| ").append(e.getKey()).append(" | ").append(ordersByEmployee.get(e.getKey())).append(" | ₹").append(String.format("%.2f", e.getValue())).append(" |\n");
            });

        return Map.of("answer", sb.toString(), "type", "employee_performance");
    }

    private Map<String, Object> getProductSalesResponse(User restaurant, LocalDateTime start, LocalDateTime end, String periodLabel) {
        List<Orders> orders = orderRepository.findByRestaurantAndCreatedAtBetweenOrderByCreatedAtDesc(restaurant, start, end);
        ...
        for (Orders o : orders) {
            ...
            for (com.probloom.model.entity.OrderItem item : o.getItems()) {
                productQty.put(item.getName(), productQty.getOrDefault(item.getName(), 0) + item.getQuantity());
                productRev.put(item.getName(), productRev.getOrDefault(item.getName(), 0.0) + (item.getPrice() * item.getQuantity()));
            }
        }
        ...
        productQty.entrySet().stream()
            ...
            .forEach(e -> {
                sb.append("| ").append(e.getKey()).append(" | ").append(e.getValue()).append(" | ₹").append(String.format("%.2f", productRev.get(e.getKey()))).append(" |\n");
            });

        return Map.of("answer", sb.toString(), "type", "product_sales");
    }

    private Map<String, Object> getSalesResponse(User restaurant, LocalDateTime start, LocalDateTime end, String periodLabel) {
        ...
        String ans = String.format("Based on the data for **%s**, your establishment generated a gross revenue of **₹%.2f**, accumulated across a total of **%d** completed orders.", periodLabel, safeTotal, safeCount);
    ...
    private Map<String, Object> getInventoryResponse(String query, User restaurant) {
        ...
        return Map.of("answer", String.format("Your inventory contains **%d** items. **%d** items are currently running low on stock.", items.size(), lowStockCount), "type", "inventory_summary");
    }

    private Map<String, Object> getEmployeeCountResponse(User restaurant) {
        return Map.of("answer", "To track specific employee counts accurately, please refer to the Workforce or Employee Management section.", "type", "text");
    }

    private Map<String, Object> getMenuSummaryResponse(User restaurant) {
        return Map.of("answer", "To see detailed menu analytics, please view the Menu Management dashboard.", "type", "text");
    }
}
